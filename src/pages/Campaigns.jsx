      ...
      <Card>
        {!campaigns ? (
          ...
          <EmptyState
            ...
            description="Create your first campaign to start configuring gifts and stock."
            ...
            <tbody>
              {campaigns.map((c) => (
                <tr key={c.id}>
                  ...
                  <td>
                    {c.total_distributed} / {c.total_stock}
                  </td>
                  <td>{new Date(c.created_at.replace(' ', 'T') + 'Z').toLocaleDateString()}</td>
                  <td>
                    <Link to={`/campaigns/${c.id}`} className="btn btn-ghost">
                      View
                    </Link>
                  </td>
                </tr>
